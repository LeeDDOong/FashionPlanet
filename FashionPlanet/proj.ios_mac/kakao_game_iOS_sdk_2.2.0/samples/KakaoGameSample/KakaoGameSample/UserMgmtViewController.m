...
- (void)showErrorMessage:(NSError *)error {
    if (error.code == KOErrorCancelled) {
        ...
        [UIAlertView showMessage:[NSString stringWithFormat:@"에러! code=%d, 에러 코드를 확인하세요", (int) error.code]];
    }
}
...
- (void)viewDidLoad {
    ...
    _menu = @[@"me", @"업데이트 프로필", @"Unlink", @"톡 프로필 보기", @"Block Message Dialog"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_doneSignup) {
        switch (indexPath.row) {
            case 0:
                [KakaoGameAPI signUpTaskWithProperties:[self userFormData] completionHandler:^(BOOL success, NSError *error) {
                    if (error) {
        ...
        switch (indexPath.row) {
            ...
            case 1: {
                [KakaoGameAPI profileUpdateTaskWithProperties:[self userFormData] completionHandler:^(BOOL success, NSError *error) {
                    if (error) {
                        [self showErrorMessage:error];
                    } else {
            ...
            case 4: {
                [KakaoGameAPI showMessageBlockDialogWithCompletionHandler:^(BOOL success, NSError *error) {
                    if (error) {
                        ...
                        [UIAlertView showMessage:@"Success Message block dialog"];
                    }
                }];
            }
                break;
