...
- (void)viewDidLoad {
    ...
    _chatContext = [MultiChatContext contextWithLimit:30];
...
- (void)requestChatList {
    ...
    _requesting = YES;
    [KakaoGameAPI multiChatListTaskWithContext:_chatContext completionHandler:^(NSArray *chats, NSError *error) {
        if (error) {
...
- (void)sendTemplateMessage {
    ...
    NSString *templateID = @"8";  // feed template message id
    [KakaoGameAPI sendMultiChatMessageTaskWithTemplateID:templateID
                                            receiverChat:chat
